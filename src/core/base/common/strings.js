...
const charCode_1 = require("./charCode");
...
function regExpContainsBackreference(regexpValue) {
    return Boolean(regexpValue.match(/([^\\]|^)(\\\\)*\\\d+/));
}
...
function doEqualsIgnoreCase(a, b, stopAt = a.length) {
    ...
    for (let i = 0; i < stopAt; i++) {
        ...
        // Any other charcode
        else if (String.fromCharCode(codeA).toLowerCase() !==
...
function startsWithUTF8BOM(str) {
    return Boolean(str && str.length > 0 && str.charCodeAt(0) === charCode_1.CharCode.UTF8_BOM);
}
...
function singleLetterHash(n) {
    ...
    n %= 2 * LETTERS_CNT;
    if (n < LETTERS_CNT) {
        return String.fromCharCode(charCode_1.CharCode.a + n);
    }
    return String.fromCharCode(charCode_1.CharCode.A + n - LETTERS_CNT);
}
