...
function isVSBufferReadableStream(obj) {
    const candidate = obj;
    return (candidate &&
        [candidate.on, candidate.pause, candidate.resume, candidate.destroy].every(fn => typeof fn === 'function'));
}
...
function toVSBufferReadableStream(stream) {
    ...
    stream.on('data', data => vsbufferStream.write(typeof data === 'string'
