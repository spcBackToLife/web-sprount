 ...
 * @returns A boolean indication if the path is a UNC path, on none-windows
 * always false.
 */
function isUNC(path) {
...
function isWindowsDriveLetter(char0) {
    return ((char0 >= charCode_1.CharCode.A && char0 <= charCode_1.CharCode.Z) ||
        (char0 >= charCode_1.CharCode.a && char0 <= charCode_1.CharCode.z));
}
...
function isRootOrDriveLetter(path) {
    ...
    if (platform_1.isWindows) {
        ...
        return (isWindowsDriveLetter(pathNormalized.charCodeAt(0)) &&
            pathNormalized.charCodeAt(1) === charCode_1.CharCode.Colon &&
