...
const extpath = require("../common/extpath");
const Platform = require("../common/platform");
const decoder_1 = require("../node/decoder");
...
const amd_1 = require("../common/amd");
...
function terminateProcess(process, cwd) {
    if (Platform.isWindows) {
        ...
        try {
            const cmd = amd_1.getPathFromAmdModule(require, './node/terminateProcess.sh');
...
(function (win32) {
    async function findExecutable(command, cwd, paths) {
        ...
        for (let pathEntry of paths) {
            ...
            if (await util_1.promisify(fs.exists)(fullPath)) {
                return fullPath;
            }
            let withExtension = fullPath + '.com';
            if (await util_1.promisify(fs.exists)(withExtension)) {
