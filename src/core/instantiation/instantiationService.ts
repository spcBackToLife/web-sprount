...
import { lowerCaseFirst } from '../../utils/util';
...
export class InstantiationService implements IInstantiationService {
  ...
  private readonly _userObjs: any; // 用户需要注入的对象
...
- 从当前服务池里找到服务，开始初始化
  ...
  private _createInstance<T>(ctor: any, args: any[] = []): T {
    ...
    const instance = <T>new ctor(...[...args, ...serviceArgs]);

    return this.injectUserObjs<T>(ctor.name, instance);
  }

  private injectUserObjs<T>(serviceName: string, instance: any): T {
    if (serviceName === 'RenderService') {
      // 仅有此服务需要注册用户代码
      Object.keys(this._userObjs).forEach(obj => {
        const isClass = (object: any) =>
        ...
        if (isClass(this._userObjs[obj])) {
          ...
          instance.__proto__[lowerCaseFirst(obj)] = new this._userObjs[obj](
            instance,
          );
        } else {
          console.log('functions:', obj);
          instance.__proto__[obj] = (...args): Promise<any> => {
